...
export function updateCardBadge(cardEl: HTMLElement, record?: JobRecord): void {
  // 查找是否已经有 Badge
  let badgeEl = cardEl.querySelector(`.${HASHED_CLASSES.badge}`) as HTMLElement | null;
  
  // 灰度逻辑：对于 viewed 和 rejected 状态，追加灰度 class，且移除/不展示 Badge (以防遮挡)
  if (record && (record.status === 'viewed' || record.status === 'rejected')) {
    cardEl.classList.add(HASHED_CLASSES.grayCard);
    if (badgeEl) {
      badgeEl.remove();
    }
    return;
  ...
  // 移除灰度 Class
  cardEl.classList.remove(HASHED_CLASSES.grayCard);
...
export function removeCardBadge(cardEl: HTMLElement): void {
  cardEl.classList.remove(HASHED_CLASSES.grayCard);
  const badgeEl = cardEl.querySelector(`.${HASHED_CLASSES.badge}`);
  if (badgeEl) {
    badgeEl.remove();
  }
}
